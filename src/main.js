...
module.exports = function main(params, options) {
  ...
  //invoke the bui-tool specified by user.
  if (params && params.length) {
    var devtool = params[0];
    return context.call('bui-' + devtool);
  }

  //invoke the bui-tool based on the project type created by initialization process.
  return fs
    .getModulePackInfo(module)
    .get('wbp')
    .get('project')
    .then(function (project) {
      if (project) return cx.call('bui-' + project);
      throw "Don't know what the type of this project is.";
    });
}
